  ...
  const entries: PortfolioEntry[] = Array.isArray(data.entries)
    ? data.entries
        .map((item) => {
          ...
          return { id: str(o.id), archived: o.archived === true };
        })
        .filter((e) => {
...
export type OrderedProject<T> = T & { archived: boolean };
...
export function orderProjects<T extends { id: string }>(
  ...
  const archivedById = new Map(config.entries.map((e) => [e.id, e.archived]));
  const orderIndex = new Map(config.entries.map((e, i) => [e.id, i]));

  return projects
    .map((p) => ({ ...p, archived: archivedById.get(p.id) ?? false }))
    .sort((a, b) => {
