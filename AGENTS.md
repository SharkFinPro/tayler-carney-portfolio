...
Untested, but *not* by design — these are gaps rather than policy, and adding a suite to one is welcome work rather than a change of direction: the remaining Server Actions in `src/app/admin` (`aiActions`, `mediaActions`, `portfolioActions`, `actions`), `auth.ts`, and `projectToBlocks` in `blocks.ts` (which is live — it is the fallback that renders any project whose `projectPage` has never been authored). The distinction matters when reading the coverage number: the first list is code we have decided not to unit-test, the second is simply work not done yet.
