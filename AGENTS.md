...
- **The block editor's three rules** — `blockHasData`, `blockSummary`, `blocksProvideH1`. The first has teeth: `BlockEditor` discards a block on save when it returns false, so a wrong answer either throws away work an admin just did or leaves empty sections on the page. All three are exhaustive switches over `BlockType`, and their tables are driven from `BLOCK_TYPES` itself rather than a copied list — so adding a block type without handling it fails here instead of shipping.
