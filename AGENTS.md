...
- **The tests are linted too**: `@vitest/eslint-plugin` runs over test files for the mistakes that make a suite *report success while testing nothing*. Eleven rules — ten errors and one warning. The errors: a committed `.only` (which cuts a file to one test with CI still green), a test with no assertion, an `expect` with no matcher, an assertion inside a conditional that may never run, an `expect` outside a test, a duplicate title, a commented-out test, an `expect` in a `.then()` on a promise the test never returns, a `return` inside a `describe` callback (which ends registration early), and `toMatchSnapshot()` in a `test.concurrent` without the local `expect`. The one warning is `no-disabled-tests`, since skipping while investigating is legitimate. `valid-expect` is set to `maxArgs: 2` because Vitest — unlike Jest — takes a failure message as `expect`'s second argument, and inside a loop that message is the only thing naming which iteration broke. One gap to know about: **an `async` describe callback is not caught by anything in the plugin** — `valid-describe-callback` sounds like it would, and doesn't.
...
- `npm run lint` — ESLint flat config (`eslint.config.mjs`). Covers the full `jsx-a11y` set and a `@vitest/eslint-plugin` block scoped to test files. Two React Compiler rules are deliberately warnings, with the reasoning in the config.
...
- **`richTextAst.ts`** — round-trip fidelity. A lossy conversion corrupts published prose invisibly. The AST→HTML→AST direction covers what the editor itself writes; a second suite covers what it *reads* — pasted markup from Word, Docs or a CMS export, where emphasis arrives as inline CSS rather than `<strong>`, lists nest, `<div>`s wrap everything, and the whitespace between tags must not become blank paragraphs. Note contentEditable makes an empty `<div>` per blank line, so an inline-only `<div>` becomes a paragraph even when empty; that is the one case distinguishing the two branches in `blockToAst`.
...
- **`useDragReorder.ts`** — the keyboard half. A drag-and-drop list that can only be reordered by dragging cannot be reordered at all without a pointer, which makes the arrow/Home/End handlers the accessible path rather than a convenience. The live-region announcement is pinned too: it is the entire feedback channel for someone who cannot see the thing move, including the two "already at the end" messages, since silence there is indistinguishable from a broken handler.
- **The block editor's three rules** — `blockHasData`, `blockSummary`, `blocksProvideH1`. The first has teeth: `BlockEditor` discards a block on save when it returns false, so a wrong answer either throws away work an admin just did or leaves empty sections on the page. A block type nobody handled is already caught by `tsc` — those switches have no `default:` and a non-optional return type — so what the suites add is the case the compiler cannot see: an arm that exists and is wrong. Their tables run over `BLOCK_TYPES` itself rather than a copied list, so those checks reach types added later too.
...
It runs weekly in [mutation.yml](.github/workflows/mutation.yml), not on the pull request: it takes about five minutes, and a surviving mutant is a prompt to write a better assertion rather than a reason to block a merge. The overall score is around 83% with the break threshold at 70, so it fails on a real regression rather than on noise. The per-file table in the log names the weak module; the HTML artifact names the surviving mutant, its line, and which tests ran without noticing — which is the question worth answering, and the one the log cannot.

Worth knowing which modules the score already rates highly, because it means their suites are doing real work rather than merely executing lines: `a11y`, `actionError`, `auth`, `cachedReads`, `ai/index`, `middleware`, `resume`, `siteUrl`, `home` and `csp` are all at 100%.

`home.ts` is there as a worked example. Its 43% came from a single shape: every assertion compared `sanitizeHome`'s output to `DEFAULT_HOME`, the same constant the production code fills gaps from — so blanking a default changed both sides at once and the suite stayed green while a fresh install rendered an empty headline. The fix was to assert *properties* of the defaults (every string non-blank, every link pointing at a real route, every repeatable section surviving its own sanitizer) rather than copying them into the test. Those cases do not go stale when someone rewords a headline, and they took the file to 100%.
