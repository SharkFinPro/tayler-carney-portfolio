...
Untested, but *not* by design — these are gaps rather than policy, and adding a suite to one is welcome work rather than a change of direction: `sitemap.ts` / `robots.ts`, both of which read `WEBSITE_URL` and silently emit a malformed document when it is unset. The distinction matters when reading the coverage number: the first list is code we have decided not to unit-test, the second is simply work not done yet.
