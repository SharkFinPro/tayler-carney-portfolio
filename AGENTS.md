...
- **The tests are linted too**: `@vitest/eslint-plugin` runs over test files for the mistakes that make a suite *report success while testing nothing* — a committed `.only` (which cuts a file to one test with CI still green), a test with no assertion, an `expect` with no matcher, an assertion inside a conditional that may never run, a duplicate title, a commented-out test, an `expect` in a `.then()` on a promise the test never returns. All are errors; `no-disabled-tests` is a warning, since skipping while investigating is legitimate. `valid-expect` is set to `maxArgs: 2` because Vitest — unlike Jest — takes a failure message as `expect`'s second argument, and inside a loop that message is the only thing naming which iteration broke. One gap to know about: **an `async` describe callback is not caught by anything in the plugin.**
